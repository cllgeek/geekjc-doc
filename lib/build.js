...
function build(commander,changeFile){
    ...
    if(commander.theme && commander.args.length > 0) template = commander.args[0];
        ...
        // 模板目录
        template = path.dirname(__dirname) + '/theme/' + pkg.gdoc.theme;
        if(!file.isDir(template)){
            template = process.cwd()+ '/' +pkg.gdoc.theme;
        }
    }
    // 皮肤目录不存在判断
    ...
    pathArr.forEach(function(item,idx){
        ...
        if(path.basename(item) === gdocmd[0]) len = 0;
        //导航菜单生成
        var navHTML = nav(gdocmd,item,pathArr,len);
        ...
        // 是否为首页判断
        gdocmd[0] === item ? isIndex = true : isIndex = false;
